...
\clearpage
\section*{Ponuka pre tímový projekt}
\addcontentsline{toc}{section}{Ponuka pre tímový projekt}

\subsection*{Identifikácia témy}

\begin{tabular}{ll}
\textbf{Názov témy:} & Virtual Data Analyst \\
\textbf{Zadávateľ témy:} & Ing. Adam Kňaze \\
\textbf{Členovia tímu:} & Peter Muzslay \\
                     & Janka Tarčáková \\
                     & Jozef Guláš \\
                     & Jana Kampová \\
                     & Tamara Uhrinová \\
...
\subsection*{Odhadované zdroje}

Časové náklady boli odhadnuté približne na jednu hodinu konzultácií týždenne. Hardvérové náklady záviseli od zvolenej architektúry riešenia. Pri cloudovom riešení bolo možné využiť služby v~rámci bezplatných vrstiev poskytovateľov, zatiaľ čo pri on-premise nasadení by bolo potrebné zabezpečiť výpočtový uzol na fakultnom serveri. Súčasťou odhadovaných zdrojov bol aj prístup cez API k~vybranému veľkému jazykovému modelu, ktorý je potrebný na generovanie SQL dotazov a~sumarizáciu výsledkov.

\subsection*{Motivácia}

Motiváciou tímu bolo preskúmať potenciál, ktorý generatívna umelá inteligencia prináša do oblasti dátovej analýzy. Cieľom bolo vytvoriť intuitívnu aplikáciu, ktorá umožní aj netechnickým používateľom klásť komplexné otázky svojim dátam v~prirodzenom jazyku. Tým sa odstraňuje bariéra medzi používateľmi a~databázami, pretože používateľ nemusí poznať SQL ani internú štruktúru databázy. Projekt zároveň predstavoval príležitosť využiť spoločné skúsenosti tímu v~oblasti backendového vývoja, frontendového vývoja, testovania a~dátového spracovania a~získať praktické skúsenosti s~jednou z~najdynamickejších technológií súčasnosti.

\subsection*{Predstavenie členov tímu}

\paragraph*{Peter Muzslay}
Počas štúdia získal skúsenosti na viacerých pozíciách, primárne ako Software Engineer a~Data Engineer. Venoval sa optimalizácii legacy PL/SQL procedúr pre rozsiahle Oracle databázy, vývoju CI/CD a~ETL pipelines a~tvorbe interných Python balíčkov pre spracovanie dát na platforme Databricks. Realizoval aj vlastný end-to-end data-engineering projekt na Google Cloud Platform s~využitím technológií BigQuery, Cloud Functions a~Terraform pre Infrastructure as Code. Tieto skúsenosti sú relevantné najmä pri návrhu dátovej vrstvy, backendovej architektúry a~nasadenia projektu.

\paragraph*{Janka Tarčáková}
Pracuje ako testerka, kde získava praktické skúsenosti s~testovaním softvéru. Počas štúdia pracovala na projektoch využívajúcich Vue, Javu, Python, PHP, HTML, CSS a~Docker. Bakalársku prácu vypracovala na tému rozpoznávania rodinného príbuzenstva medzi dvojicami obrazov tvárí, kde pracovala v~Pythone, pripravovala dataset a~riešila tréning a~testovanie modelu. Jej skúsenosti sú prínosné najmä pri testovaní aplikácie, spracovaní dát a~overovaní kvality riešenia.

\paragraph*{Jana Kampová}
Má skúsenosti s~tvorbou webových aplikácií v~technológiách HTML, CSS, JavaScript, TypeScript, Angular a~Vue. Pracovala aj s~Dockerom a~PHP na strane backendu. V~rámci webových projektov realizovala galérie, formuláre, hlasovacie aplikácie a~systémy na overovanie vedomostí študentov vrátane autentifikácie. Vyskúšala si aj vývoj mobilnej hry využívajúcej gyroskop a~hry založenej na socketovej komunikácii.

\paragraph*{Jozef Guláš}
Pracoval na projektoch s~využitím technológií JavaScript, PHP a~Java. V~bakalárskej práci upravoval a~vizualizoval GPS trasy v~jazyku Python s~využitím knižníc Pandas, Folium a~NumPy. Jeho skúsenosti sú využiteľné pri práci s~dátami, vizualizáciách a~implementácii aplikačnej logiky.

\paragraph*{Tamara Uhrinová}
Pracovala na rôznych projektoch, kde využívala technológie .NET, React, SQLite, ASP.NET Core, Java, C\# a~Python. V~rámci bakalárskej práce vyvíjala webovú aplikáciu na vyhľadávanie bilingválnych škôl a~testovací portál, kde sa používatelia môžu registrovať, riešiť testy a~sledovať svoje výsledky. Tieto skúsenosti sú prínosné najmä pri návrhu používateľského rozhrania, autentifikácie a~aplikačnej logiky.
...
\subsection{Rozsah dokumentácie}

Táto dokumentácia obsahuje oficiálne zadanie projektu, vypracovanú ponuku, ciele riešenia, analýzu problému, opis použitých algoritmov a~metód, dokumentáciu k~jednotlivým programovým častiam, výsledky riešenia, porovnanie so známymi existujúcimi nástrojmi a~použitú literatúru a~zdroje spracovávaných textov.

Súčasťou projektovej dokumentácie je aj webová stránka tímu dostupná na adrese \url{https://peter115342.github.io/TP25_26_webpage/}. Na tejto stránke sú zverejnené základné informácie o~projekte a~zápisnice zo stretnutí, ktoré dokumentujú priebeh práce tímu počas semestra.
...
Virtual Data Analyst tieto bariéry odstraňuje a~sprístupňuje dátovú analýzu širšiemu okruhu používateľov.
